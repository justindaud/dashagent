...
export default function Dashboard() {
  ...
  const formatDateRange = (from: Date | null | undefined, to: Date | null | undefined, format: "short" | "medium" | "long" = "medium") => {
    ...
    if (format === "short") {
      ...
      return `${from.getDate().toString().padStart(2, "0")}/${(from.getMonth() + 1).toString().padStart(2, "0")}/${from.getFullYear()} - ${to
        .getDate()
        .toString()
        .padStart(2, "0")}/${(to.getMonth() + 1).toString().padStart(2, "0")}/${to.getFullYear()}`;
    } else {
  ...
  const [dimensions, setDimensions] = useState<{ segment: string[]; room_type: string[]; local_region: string[]; nationality: string[] }>({
    segment: [],
    room_type: [],
    local_region: [],
    nationality: [],
  ...
  const fetchDashboardData = async () => {
    try {
      const [statsRes, uploadsRes] = await Promise.all([axios.get(`${API_BASE}/dashboard/stats`), axios.get(`${API_BASE}/dashboard/recent-uploads`)]);
              ...
              <ResizablePanelGroup direction="horizontal" className="min-h-[600px] rounded-lg border">
                ...
                <ResizablePanel defaultSize={50} minSize={30}>
                  <div className="py-4 px-8 space-y-4">
                    <h3 className="text-xl font-semibold mb-2">Panel A</h3>
                    <div className="mb-4">
                      ...
                      <AccordionItem value="filters">
                        ...
                        <AccordionContent className="space-y-4">
                          <div className="space-y-2">
                            ...
                            <select
                              value={panelA.group_by}
                              onChange={(e) => setPanelA((p) => ({ ...p, group_by: e.target.value as GroupBy }))}
                        ...
                        <AccordionContent className="space-y-4">
                          <div className="grid grid-cols-2 gap-4">
                            <div className="space-y-2">
                              ...
                              <input
                                ...
                                max={50}
                                value={panelA.topN || 6}
                                onChange={(e) => setPanelA((p) => ({ ...p, topN: Number(e.target.value) }))}
                                className="w-full border rounded px-2 py-1 mt-1 hover:border-primary"
                              />
                            </div>
                            <div className="space-y-2">
                              ...
                              <input
                                type="number"
                                min={1}
                                value={panelA.totalRooms}
                                onChange={(e) => setPanelA((p) => ({ ...p, totalRooms: Number(e.target.value) }))}
                                className="w-full border rounded px-2 py-1 mt-1 hover:border-primary"
                              />
                            </div>
                          </div>
                          <div className="flex items-center gap-2">
                            <input
                              id="includeOtherA"
                              type="checkbox"
                              checked={panelA.includeOther}
                              onChange={(e) => setPanelA((p) => ({ ...p, includeOther: e.target.checked }))}
                            />
                            <label htmlFor="includeOtherA" className="text-sm text-gray-600">
                              Include Other
                            </label>
                          </div>
                        </AccordionContent>
                          ...
                          <div className="mb-6">
                            <div className="text-center p-4 bg-green-50 rounded-lg mb-4">
                              <div className="text-2xl font-bold text-green-600">
                                {computeOccupancyPct(panelA.data, panelA.totalRooms).toFixed(1)}%
                              </div>
                ...
                <ResizablePanel defaultSize={50} minSize={30}>
                  <div className="py-4 px-8 space-y-4">
                    <h3 className="text-xl font-semibold mb-2">Panel B</h3>
                      ...
                      <AccordionItem value="filters-b">
                        ...
                        <AccordionContent className="space-y-4">
                          <div className="space-y-2">
                            <label className="text-sm text-gray-600 font-medium">Group By</label>
                            <select
                              value={panelB.group_by}
                              onChange={(e) => setPanelB((p) => ({ ...p, group_by: e.target.value as GroupBy }))}
                              className="w-full border rounded px-2 py-1 mt-1 hover:border-primary"
                            >
                              <option value="none">None</option>
                              <option value="segment">Segment</option>
                        ...
                        <AccordionContent className="space-y-4">
                          <div className="grid grid-cols-2 gap-4">
                            <div className="space-y-2">
                              ...
                              <input
                                ...
                                max={50}
                                value={panelB.topN || 6}
                                onChange={(e) => setPanelB((p) => ({ ...p, topN: Number(e.target.value) }))}
                                className="w-full border rounded px-2 py-1 mt-1 hover:border-primary"
                              />
                            </div>
                            <div className="space-y-2">
                              ...
                              <input
                                type="number"
                                min={1}
                                value={panelB.totalRooms}
                                onChange={(e) => setPanelB((p) => ({ ...p, totalRooms: Number(e.target.value) }))}
                                className="w-full border rounded px-2 py-1 mt-1 hover:border-primary"
                              />
                            </div>
                          </div>
                          <div className="flex items-center gap-2">
                            <input
                              id="includeOtherB"
                              type="checkbox"
                              checked={panelB.includeOther}
                              onChange={(e) => setPanelB((p) => ({ ...p, includeOther: e.target.checked }))}
                            />
                            <label htmlFor="includeOtherB" className="text-sm text-gray-600">
                              Include Other
                            </label>
                          </div>
                        </AccordionContent>
                    ...
                    <div className="flex gap-2 mb-4">
                      <Button onClick={() => fetchAnalytics("B")} disabled={!compareOn}>
                        {panelB.loading ? "Loading…" : "Apply B"}
                      </Button>
                      ...
                      {panelB.data ? (
                        <>
                          ...
                          <div className="mb-6">
                            <div className="text-center p-4 bg-green-50 rounded-lg mb-4">
                              <div className="text-2xl font-bold text-green-600">
                                {computeOccupancyPct(panelB.data, panelB.totalRooms).toFixed(1)}%
                              </div>
                              <div className="text-sm text-gray-600">Occupancy Rate</div>
                              <div className="text-xs text-gray-500 mt-1">
                                {formatDateRange(panelB.dateRange?.from, panelB.dateRange?.to, "medium")}
                    ...
                    <>
                      ...
                      <div className="mb-6">
                        <div className="text-center p-4 bg-blue-50 rounded-lg mb-4">
                          <div className="text-2xl font-bold text-blue-600">{formatCurrency(panelA.data.totals.revenue_sum)}</div>
                          <div className="text-sm text-gray-600">Total Revenue</div>
                          <div className="text-xs text-gray-500 mt-1">{formatDateRange(panelA.dateRange?.from, panelA.dateRange?.to, "medium")}</div>
                        </div>
                        <PieChartContainer
                          data={panelA.data.breakdown}
                          dataKey="revenue_sum"
      ...
      <Dialog open={isChatbotOpen} onOpenChange={setIsChatbotOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <Button onClick={() => setIsChatbotOpen(false)} className="absolute right-4 top-4">
              <X className="w-4 h-4" />
            </Button>
          </DialogHeader>
