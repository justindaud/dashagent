...
def create_session(payload: SessionCreate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    prepared = ctl.create_session_controller(db, user_id=str(current_user.user_id))
    return success(
        message="Session prepared successfully",
        data=SessionPreparedOut.model_validate(prepared),
        status_code=201,
    )

@router.get("/")
def list_my_sessions(db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    sessions = ctl.list_my_sessions_controller(db, user_id=str(current_user.user_id))
...
def get_my_session(session_id: str, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    return success(
        ...
        data=SessionOut.model_validate(sess),
        status_code=200,
    )
